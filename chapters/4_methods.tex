...
Following feature selection (see Supplementary Material for details), the following features were included in the model: prior disability, stroke severity, age, congestive heart failure, hypertension, diabetes, AFib anticoagulant, AFib diagnosis, onset to thrombolysis time, onset to thrombectomy time, stroke team, ethnicity. Onset to thrombolysis time, onset to thrombectomy time were modelled as either continuous variables, or according to early/late groups as above.

A secondary mode was fitted to the estimated treatment benefit, for treated patients, to better understand the relationship between patient and treatment features, and the benefit achieved. 

\subsubsection{Causal Forest}

We used an orthogonal causal forest \cite{oprescu2019orthogonal} to estimate conditional average treatment effects—that is, the expected effect of treatment for patients with particular combinations of baseline characteristics. This is a combination of three tree-based models. First, flexible LightGBM prediction models \cite{ke2017lightgbm} estimated each patient’s expected outcome and likelihood of receiving treatment from the measured baseline features. The remaining residual variation in treatment and outcome, after accounting for these features, is passed to a Causal Forest tree algorithm. This approach allows for complex and non-linear relationships among features, and the Causal Forest maximises prediction accuracy of treatment effects for individuals. This estimator is considered \textit{doubly robust} because it combines two complementary models: an outcome model, which estimates expected outcomes given patients’ baseline characteristics, and a treatment-assignment model, which estimates each patient’s probability of receiving treatment given those characteristics. Under the usual causal assumptions, the estimated treatment effect remains consistent if \textit{either} the outcome model or the treatment-assignment model is correctly specified. This property provides some protection against misspecification of the full causal model.

The features used in the model were as the Explainable S-Learner, with onset-to-treatment times being grouped by early/late.